...
public List<Match> mostrarMatches(Usuario usuario) throws Exception {
    if (usuario == null) {
        throw new Exception("Debe especificar un usuario para mostrar sus matches.");
    }

    System.out.println("🔎 Mostrando matches del usuario con ID: " + usuario.getIdUsuario());

    // Obtener matches donde el usuario participa como A
    List<Match> listaA = matchDAO.buscarPorUsuarioA(usuario);

    // Obtener matches donde el usuario participa como B
    List<Match> listaB = matchDAO.buscarPorUsuarioB(usuario);

    // Combinar ambas listas
    List<Match> matchesTotales = new ArrayList<>();

    if (listaA != null) {
        matchesTotales.addAll(listaA);
    }

    if (listaB != null) {
        matchesTotales.addAll(listaB);
    }

    // Mostrar por consola los resultados (para depuración)
    System.out.println("✅ Matches encontrados: " + matchesTotales.size());

    return matchesTotales;
}
